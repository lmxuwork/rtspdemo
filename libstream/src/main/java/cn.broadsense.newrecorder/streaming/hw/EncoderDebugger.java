...
package cn.broadsense.newrecorder.streaming.hw;
...
public class EncoderDebugger {
    ...
    private void debug() {
        ...
        // Tries available encoders 尝试使用编码器
        for (int i = 0; i < encoders.length; i++) {
            for (int j = 0; j < encoders[i].formats.length; j++) {
                ...
                // Converts from NV21 to YUV420 with the specified parameters 转换从NV21 YUV420与指定参数
                mNV21.setSize(mWidth, mHeight);
    ...
    private long decode(boolean withPrefix) {
        ...
        while (elapsed < 3000000) {
            ...
            Log.e(TAG, "====================decOutputIndex===============" + decOutputIndex);
