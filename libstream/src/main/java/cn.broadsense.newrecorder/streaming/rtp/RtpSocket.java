...
package cn.broadsense.newrecorder.streaming.rtp;

import android.os.SystemClock;

import cn.broadsense.newrecorder.streaming.rtcp.SenderReport;
...
public class RtpSocket implements Runnable {
	...
	public void setDestination(InetAddress dest, int dport, int rtcpPort) {
		Log.e("SetDest", "Set Dest: " + dest + " port:" + dport + " rtcpPort:" + rtcpPort);
	...
	public void run() {
		...
		try {
			...
			while (mBufferCommitted.tryAcquire(4, TimeUnit.SECONDS)) {
				...
				if (mCount++>30) {//TODO:
					//Log.e(TAG, "russssssssssssssssssssssssn.......  " + mTransport);
					if (mTransport == TRANSPORT_UDP) {
						//Log.e(TAG, " send u  to udp....mBufferOut:"+mBufferOut);
						try {
			//				DatagramPacket pkg = mPackets[mBufferOut];
			//				Log.e(TAG, " send udp data: addr: " + pkg.getAddress() + "  dataLen:" + pkg.getData().length + "  socketAddr:" + pkg.getSocketAddress() + " pkg:" + pkg.getLength());
							mSocket.send(mPackets[mBufferOut]);
						}catch(Throwable e){
							Log.e(TAG, "send udp fail:"+e.getMessage(), e);
						}
					} else {
						sendTCP();
					}
